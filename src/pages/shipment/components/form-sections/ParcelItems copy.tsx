...
import { Button, Input, Select, SelectItem, Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter } from '@heroui/react'
...
const ParcelItems = ({ parcelIndex, control, register, errors }: ParcelItemsProps) => {
  ...
  const openMaterialModal = (itemIndex: number) => {
    alert("clicked material list select");     
    setIsModalOpen(true)
  ...
  const handleMaterialSelect = (selectedMaterial: MaterialData) => {
    if (currentItemIndex !== null) {
      // Auto-fill description and SKU from selected material
      const descInput = document.querySelector(`input[name="parcels.${parcelIndex}.parcel_items.${currentItemIndex}.description"]`) as HTMLInputElement
      const skuInput = document.querySelector(`input[name="parcels.${parcelIndex}.parcel_items.${currentItemIndex}.sku"]`) as HTMLInputElement
      
      if (descInput) {
        descInput.value = selectedMaterial.description
        descInput.dispatchEvent(new Event('input', { bubbles: true }))
      }
      if (skuInput) {
        skuInput.value = selectedMaterial.sku
